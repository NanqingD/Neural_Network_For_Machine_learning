...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % Backpropagation
  delta_hid_to_class = class_output - data.targets;
  ret.hid_to_class = 1 / n * delta_hid_to_class * hid_output' + wd_coefficient * model.hid_to_class;
  delta_input_to_hid = (model.hid_to_class' * delta_hid_to_class) .* hid_output .* (1 - hid_output);
  ret.input_to_hid = 1 / n * delta_input_to_hid * data.inputs' + wd_coefficient * model.input_to_hid;
end
